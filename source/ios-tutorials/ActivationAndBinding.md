...
This demo is build based on the [ImportSDKDemo](https://github.com/DJI-Mobile-SDK-Tutorials/iOS-ImportAndActivateSDKInXcode) Github Sample, you can check the [Integrate SDK into Application](./workflow-integrate.html) tutorial to learn how to register the application using DJI Mobile SDK.
...
Now, let's build and run the project, connect the demo application to your aircraft (Please check the [Run Application](../tutorials-and-samples/iOS/ActivationAndBinding/workflow-run.html) for more details) and check the features we have implemented so far.
